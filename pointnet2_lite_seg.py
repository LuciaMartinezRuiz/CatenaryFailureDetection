...
def focal_loss(logits, targets, weight=None, gamma=2.0, ignore_index=-1):
    """
    Focal Loss multiclase con ignore_index.
    logits:  (M, ncls)
    targets: (M,)
    """
    ...
    ce = F.cross_entropy(logits_v, targets_v, weight=weight, reduction="none")
    pt = torch.exp(-ce)
...
def main():
    ...
    ap.add_argument("--class_sampling_power", type=float, default=0.0, help="0 = muestreo uniforme; >0 sobre-muestrea clases raras.")
    ...
    ap.add_argument("--class_weight_cap", type=float, default=5.0)

    # ---- sparse supervision ----
    ap.add_argument("--label_keep_ratio", type=float, default=1.0,
                    help="Fracción de etiquetas disponibles (1.0 = full). Ej: 0.001=0.1%%")
    ap.add_argument("--sparse_n_labeled", type=int, default=32,
                    help="Cuántos puntos etiquetados FORZAMOS dentro de cada bloque muestreado (para que haya gradiente).")
    ap.add_argument("--ignore_index", type=int, default=-1,
                    help="Valor de etiqueta para puntos sin supervisión.")
    ap.add_argument("--min_labeled_per_tile", type=int, default=10,
                    help="Asegura al menos N labels por tile para evitar tiles sin supervisión.")
    ...
    # Precomputar "puntos etiquetados disponibles" por tile (sparse mask fijo por tile)
    labeled_idx_by_tile = None
    if args.label_keep_ratio < 1.0:
        labeled_idx_by_tile = []
        for tid, yt in enumerate(y_train):
            rs_tile = np.random.RandomState(args.seed * 1000 + tid * 17 + 7)
            keep = rs_tile.rand(len(yt)) < float(args.label_keep_ratio)
            idx_keep = np.where(keep)[0]
            if len(idx_keep) < int(args.min_labeled_per_tile):
                # fuerza mínimo para evitar tiles sin labels
                extra = rs_tile.choice(len(yt), size=int(args.min_labeled_per_tile) - len(idx_keep), replace=False if len(yt) >= int(args.min_labeled_per_tile) else True)
                idx_keep = np.unique(np.concatenate([idx_keep, extra]))
            labeled_idx_by_tile.append(idx_keep.astype(np.int64))
        ...
        print("[sparse] full supervision (label_keep_ratio=1.0)")
    ...
    for ep in range(1, args.epochs + 1):
        ...
        for _ in range(steps_per_epoch):
            ...
            for _b in range(args.batch_size):
                ...
                # ---- FULL supervision (igual que antes) ----
                if args.label_keep_ratio >= 1.0:
                ...
                # ---- SPARSE supervision ----
                    ...
                    k = int(min(max(1, args.sparse_n_labeled), args.n_points))
                    lab_idx = labeled_idx_by_tile[tid]
                    if len(lab_idx) == 0:
                        lab_sel = rs_global.choice(len(Xt), size=k, replace=True)
                    else:
                        lab_sel = rs_global.choice(lab_idx, size=k, replace=True)

                    rem = args.n_points - k
                    if rem > 0:
                        unlab_sel = rs_global.choice(len(Xt), size=rem, replace=(len(Xt) < rem))
                        idx = np.concatenate([lab_sel, unlab_sel], axis=0)
                        xb = Xt[idx].copy()
                        yb = np.full((args.n_points,), int(args.ignore_index), dtype=np.int64)
                        yb[:k] = yt[lab_sel].copy()
                    else:
                        idx = lab_sel
                        xb = Xt[idx].copy()
                        yb = yt[lab_sel].copy()

                    # shuffle dentro del bloque
                    perm = rs_global.permutation(args.n_points)
                    xb = xb[perm]
                    yb = yb[perm]
            ...
            # contar labels efectivas (solo en sparse)
            labeled_counts.append(int((y2 != int(args.ignore_index)).sum().item()))

            if args.loss == "focal":
                loss = focal_loss(logits2, y2, weight=ce_weight, gamma=args.focal_gamma, ignore_index=int(args.ignore_index))
                if loss is None:
                    continue
            else:
                loss = F.cross_entropy(logits2, y2, weight=ce_weight, ignore_index=int(args.ignore_index))

            loss.backward()
            opt.step()
            losses.append(float(loss.item()))

        # Validación estable (full labels)
        model.eval()
        oa, miou, miou_p = eval_on_arrays(
            model, X_val[0], y_val[0],
            ncls=ncls,
        ...
        mean_lab = float(np.mean(labeled_counts)) if len(labeled_counts) else 0.0
        print(f"[ep {ep:03d}] loss={mean_loss:.4f} mean_labeled/step={mean_lab:.1f} val_OA={oa:.3f} val_mIoU={miou:.3f} val_mIoU_present={miou_p:.3f}")

    # Guardar ckpt con metadatos
    ...
    ckpt = {
        ...
        "label_keep_ratio": float(args.label_keep_ratio),
        "sparse_n_labeled": int(args.sparse_n_labeled),
